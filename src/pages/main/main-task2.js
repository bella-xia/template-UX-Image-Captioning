...
function Main2Container() {
    ...
    return [1, 2, 3].map((columnNumber) => (
      <td key={columnNumber} style={{ marginRight: "10px" }}>
        <input
          ...
          value={columnNumber}
          onChange={() => handleRadioChange(rowNumber, columnNumber)}
          disabled={columnDisabled[columnNumber]}
        />
      </td>
    ));
  };
  ...
  const handleRadioChange = (rowNumber, columnNumber) => {
    console.log(`Survey Selection: Row ${rowNumber}, Column ${columnNumber}`);

    // Check if the column is already selected in other rows
    if (
      Object.values(selectedColumns).includes(columnNumber) &&
      !Object.keys(selectedColumns).some(
        (key) => key !== rowNumber && selectedColumns[key] === columnNumber
      )
    ) {
    ...
    // Check if the column is already selected in the current row
    if (selectedColumns[rowNumber] === columnNumber) {
      // Disable the column and clear selection
      setColumnDisabled((prev) => ({ ...prev, [columnNumber]: true }));
      setSelectedColumns((prev) => ({ ...prev, [rowNumber]: null }));
    } else {
      // Enable the previously disabled column (if any)
      if (selectedColumns[rowNumber] !== null) {
        setColumnDisabled((prev) => ({
          ...prev,
          [selectedColumns[rowNumber]]: false,
        }));
      }

      // Enable the newly selected column
      setColumnDisabled((prev) => ({ ...prev, [columnNumber]: false }));
      setSelectedColumns((prev) => ({ ...prev, [rowNumber]: columnNumber }));
    }
  };
  ...
  const nextChange = () => {
    ...
    if (count < totalImages) {
      ...
      if (finishCounter === 1) {
        ...
        alert(
          "If you click Next then you will be finishing the scoring for the amount of details. Click on Next again if you are finished."
        );
        setFinishCounter(1);
      }
    }

    // reset the selectedColumns state
    setSelectedColumns({
      1: null,
      2: null,
      3: null,
  ...
  return (
    <>
      <h1>
        {" "}
        Considering the image and each caption, rank the three captions in order
        of amount of details, with 1 being the most amount of details and 3
        being the least amount of details, based solely on the correctness of
        the details they contain. Ignore length and amount of detail.
      </h1>
      {render ? (
